...
\title{\textit{Red Oak}: Proactive Policy Generation}
...
\end{lstlisting}
  ...
  For example, a packing coming from \lstinline|ipsrc| 150 and \lstinline|ipdst| 10 would be matched by the second rule, and dropped on the switch.
Subsequently, on a packet with \lstinline|ipsrc| 10 and \lstinline|ipdst| 150, the first rules would be matched, evaluating the decision tree would add tuple $(10,150)$ to relation \lstinline|conn|, the packet would be forwarded according to the forwarding decision \lstinline|Deliver|, and the concrete rules would updated as follow:
...
ipsrc in ([101, 1000] $\cap$ 150), ipdst = 10
    --> Deliver
ipsrc in ([101, 1000] $\cap$ 200), ipdst = 9
    --> Deliver
ipsrc in ([101, 1000] $\cap$ 8), ipdst = 150
    --> Deliver
 \end{lstlisting}
which reduces to:
\begin{lstlisting}
