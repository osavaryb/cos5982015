...
Src = 200, Dst = 9 $\mapsto$ Deliver
 \end{lstlisting}
Note the omission of the third concrete rule, resulting from \lstinline|Src| having an empty range. We can now concretize the second symbolic rule without doing any work, since checking for relation membership is already done by all the higher priority rules. The final concrete forwarding rules are now:
\begin{lstlisting}[mathescape]
Src = 150, Dst = 10 $\mapsto$ Deliver
Src = 200, Dst = 9 $\mapsto$ Deliver
Src $\in$ [101, 1000] $\mapsto$ Drop
